"""Distributional Soft Actor-Critic (DSAC) for risk-sensitive continuous control.

DSAC extends SAC with distributional critics that learn the full return distribution
using quantile regression. This enables risk-sensitive policy optimization via
CVaR, VaR, and other risk measures.

Key differences from SAC:
- Twin Q-networks output N quantile values instead of scalar Q-values
- Uses quantile Huber loss for critic training
- Actor can use risk-sensitive values (CVaR, VaR, mean-std) for optimization
- Same actor architecture as SAC (Gaussian policy with tanh squashing)

Reference: Ma et al. (2020) "DSAC: Distributional Soft Actor Critic for
...
from stoix.utils.loss import dsac_quantile_loss, risk_sensitive_value
...
def get_learner_fn(
    ...
    """Get the learner function for DSAC."""
    ...
    # Fixed quantile fractions (midpoints of uniform intervals)
    tau = jnp.array(
        [(2 * i - 1) / (2 * config.system.num_quantiles) for i in range(1, config.system.num_quantiles + 1)]
    )
    ...
    def _update_step(
        ...
        def _update_epoch(update_state: Tuple, _: Any) -> Tuple:
            ...
            def _q_loss_fn(
                ...
                """Distributional Q-loss using quantile regression."""
                # Get current quantiles: (batch, 2, num_quantiles) -> twin Q-networks
                q_quantiles = q_apply_fn(q_params, transitions.obs, transitions.action)
                ...
                # Get target quantiles: (batch, 2, num_quantiles)
                target_q_quantiles = q_apply_fn(target_q_params, transitions.next_obs, next_action)

                # Take minimum per quantile (distributional clipped double Q)
                target_q_min = jnp.min(target_q_quantiles, axis=-2)  # (batch, num_quantiles)

                # Subtract entropy bonus from each quantile
                target_q_min = target_q_min - alpha * next_log_prob[:, None]

                # Bellman backup for each quantile
                target_quantiles = jax.lax.stop_gradient(
                    transitions.reward[:, None]
                    + (1.0 - transitions.done[:, None]) * config.system.gamma * target_q_min
                )

                # Compute loss for both Q-networks
                q1_quantiles = q_quantiles[..., 0, :]  # (batch, num_quantiles)
                q2_quantiles = q_quantiles[..., 1, :]  # (batch, num_quantiles)

                q1_loss = dsac_quantile_loss(q1_quantiles, target_quantiles, tau, config.system.huber_loss_parameter)
                q2_loss = dsac_quantile_loss(q2_quantiles, target_quantiles, tau, config.system.huber_loss_parameter)

                q_loss = q1_loss + q2_loss

                loss_info = {
                    "q_loss": q_loss,
                    "q1_mean": jnp.mean(q1_quantiles),
                    "q2_mean": jnp.mean(q2_quantiles),
                }
            ...
            def _actor_loss_fn(
                ...
                """Actor loss with optional risk-sensitive value estimation."""
                actor_policy = actor_apply_fn(actor_params, transitions.obs)
                action = actor_policy.sample(seed=key)
                log_prob = actor_policy.log_prob(action)

                # Get quantiles from both Q-networks
                q_quantiles = q_apply_fn(q_params, transitions.obs, action)

                # Take minimum per quantile
                q_min_quantiles = jnp.min(q_quantiles, axis=-2)  # (batch, num_quantiles)

                # Apply risk measure to get scalar Q-value
                q_value = risk_sensitive_value(
                    q_min_quantiles,
                    tau,
                    config.system.risk_measure,
                    config.system.risk_param,
                )
...
def learner_setup(
    ...
    def create_distributional_q_network(cfg: DictConfig) -> CompositeNetwork:
        """Create a distributional Q-network that outputs quantiles."""
        ...
        q_network_head = hydra.utils.instantiate(
            cfg.network.q_network.critic_head,
            num_quantiles=cfg.system.num_quantiles,
        )
        return CompositeNetwork([q_network_input, q_network_torso, q_network_head])

    double_q_network = MultiNetwork([
        create_distributional_q_network(config),
        create_distributional_q_network(config)
    ])
...
@hydra.main(
    ...
    config_name="default_ff_dsac.yaml",
    version_base="1.2",
)
def hydra_entry_point(cfg: DictConfig) -> float:
    ...
    print(f"{Fore.CYAN}{Style.BRIGHT}DSAC experiment completed{Style.RESET_ALL}")
    return eval_performance
