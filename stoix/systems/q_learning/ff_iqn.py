"""Implicit Quantile Networks (IQN) for distributional Q-learning.

IQN learns an implicit representation of the return distribution by sampling
quantile fractions (tau) at runtime and embedding them using cosine features.
This allows learning the full distribution without pre-specifying quantile locations.

Key differences from QR-DQN:
- Samples tau values uniformly at runtime instead of using fixed quantiles
- Uses cosine embeddings to represent tau values
- Can use different numbers of tau samples for online (N) and target (N') networks
- Supports risk-sensitive policies via tau distortion
...
from stoix.utils.loss import iqn_q_learning
...
def get_warmup_fn(
    ...
    def warmup(
        ...
        def _env_step(
            ...
            # SELECT ACTION
            key, policy_key, tau_key = jax.random.split(key, 3)
            # IQN needs a key for tau sampling - pass via head_kwargs
            actor_policy, _, _ = q_apply_fn(
                q_params.online, last_timestep.observation, head_kwargs={"key": tau_key}
            )
    ...
    def _update_step(
        ...
        def _update_epoch(update_state: Tuple, _: Any) -> Tuple:
            ...
            def _q_loss_fn(
                ...
                # Sample tau for online and target networks
                key, tau_key, tau_prime_key = jax.random.split(key, 3)
                batch_size = transitions.obs.shape[0]

                # Sample tau for online network (N samples)
                tau = jax.random.uniform(tau_key, (batch_size, config.system.num_tau_samples))

                # Sample tau' for target network (N' samples)
                tau_prime = jax.random.uniform(
                    tau_prime_key, (batch_size, config.system.num_tau_prime_samples)
                )

                # Get quantiles from online network - pass tau via head_kwargs
                _, q_quantiles_tm1, _ = q_apply_fn(
                    q_params, transitions.obs, head_kwargs={"tau": tau}
                )

                # Get quantiles from target network
                _, q_quantiles_t, _ = q_apply_fn(
                    target_q_params, transitions.next_obs, head_kwargs={"tau": tau_prime}
                )
...
class EvalActorWrapper:
    """Wrapper for evaluation that provides a consistent interface."""
    actor: Actor
    eval_key: chex.PRNGKey = None

    def apply(self, params: FrozenDict, x: Observation) -> distrax.EpsilonGreedy:
        # For evaluation, use a fixed key for reproducibility
        # The key is split from the main eval key in evaluator_setup
        policy, _, _ = self.actor.apply(params, x, head_kwargs={"key": self.eval_key})
        return policy
...
def learner_setup(
    ...
    # PRNG keys.
    key, q_net_key, eval_tau_key = jax.random.split(keys[0], 3)
    ...
    eval_q_network = EvalActorWrapper(actor=eval_q_network, eval_key=eval_tau_key)
    ...
    # IQN needs a key for tau sampling during init - pass via head_kwargs
    init_key = jax.random.PRNGKey(0)
    q_online_params = q_network.init(q_net_key, init_x, head_kwargs={"key": init_key})
...
@hydra.main(
    ...
    config_name="default_ff_iqn.yaml",
    version_base="1.2",
)
def hydra_entry_point(cfg: DictConfig) -> float:
    ...
    print(f"{Fore.CYAN}{Style.BRIGHT}IQN experiment completed{Style.RESET_ALL}")
    return eval_performance
